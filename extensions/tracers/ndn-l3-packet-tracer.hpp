...
#ifndef CCNX_PACKET_L3_TRACER_H
#define CCNX_PACKET_L3_TRACER_H
 ...
 */
class L3PacketTracer : public L3Tracer {
public:
  /**
   ...
   */
  L3PacketTracer(shared_ptr<std::ostream> os, Ptr<Node> node);
   ...
   */
  L3PacketTracer(shared_ptr<std::ostream> os, const std::string& node);

  /**
   * @brief Destructor
   */
  virtual ~L3PacketTracer();
...
private:
  virtual void
  printLog(uint32_t faceId, std::string faceDescr, std::string type, std::string name, uint32_t kilobytes);
